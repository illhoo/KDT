#!/usr/bin/env python3
"""
KDT 야간 모니터링 — shortlist + 규칙 기반 curation 자동 생성 [v7.1 혼재매체 뉴스 화이트리스트]
...
AD_RULES = [
    ...
    # 구인·모집 — v6.3: 준광고가 아니라 완전 제외.
    #   "한인타운 일식점 서버분 모집합니다"처럼 동포 키워드(+5)를 달고 이관까지
    #   올라오던 구인광고를 원천 차단. 구인글은 발제 회의에서 볼 가치가 0이다.
    (5, r"모집|구인|구합니다|구함|급구|채용\s*공고|알바|파트\s*타임|풀\s*타임"),
...
# ══════════════════════════════════════════════════════════════════════════════
# 혼재 매체 뉴스 화이트리스트 (v7 신설) — 광고 차단의 주력
#
#   문제: v6까지는 "광고 표현"을 열거해 차단했다. 표현이 무한해서 매일 뚫렸다.
#         방1+화1 → 원베드 → Studio → 술집 매물 → 아파트 → 음악회 초대…
#         게다가 링크가 Google News 리다이렉트로 남아 URL 차단은 발동조차 안 됐다.
#
#   전환: RadioKorea처럼 뉴스와 생활정보 게시판이 같은 RSS로 나오는 매체는
#         "광고를 뺀다"가 아니라 "기사만 넣는다".
#         뉴스 제목에는 반드시 서술어가 있다(실종·체포·발표·열려·했다·밝혀).
#         광고 제목은 명사구로 끝난다(한인타운 술집 매물 / 넓은 사이즈의 아파트).
#         광고 표현은 무한하지만 기사 서술어는 유한하다. 잡을 수 있는 쪽을 잡는다.
#
#   적용: MIXED_SOURCE_MEDIA 매체의 기사는 뉴스 서술어가 없으면 제외.
#         다른 매체는 영향 없음(순수 뉴스 매체라 이 규칙 불필요).
# ══════════════════════════════════════════════════════════════════════════════

MIXED_SOURCE_MEDIA = [
    "radiokorea", "라디오코리아",   # 뉴스 + 벼룩시장·구인·부동산 게시판 혼재
]

# 뉴스 서술어 — 한국어 기사 제목의 종결·연결 어미와 보도 동사
NEWS_PREDICATE_RE = re.compile(
    # 사건·사고
    r"실종|사망|숨져|숨진|부상|체포|구속|기소|송치|피소|입건|검거|수배|"
    r"화재|폭발|붕괴|충돌|추락|침수|총격|피격|강도|절도|폭행|성추행|사기|피해|"
    # 수사·사법
    r"수사|조사|압수|기소|판결|선고|소송|고소|고발|재판|항소|무죄|유죄|벌금|"
    r"적발|단속|철퇴|제재|경고|과징금|"
    # 정치·행정
    r"발표|공개|밝혀|밝혔|전해|알려|확인|추진|검토|결정|확정|승인|허가|가결|부결|"
    r"선출|당선|취임|사퇴|사임|출마|임명|위촉|해임|경질|"
    r"회담|협약|체결|합의|서명|방문|접견|면담|청문회|국감|"
    # 사회·경제
    r"급등|급락|폭등|폭락|인상|인하|증가|감소|늘어|줄어|하락|상승|"
    r"논란|반발|비판|규탄|촉구|요구|항의|시위|파업|집회|성명|"
    r"우려|경고|비상|위기|경보|주의보|"
    # 행사·수상 (동사형만 — 명사 '초대·안내'는 제외)
    r"열려|열린|열렸|개최했|개최한|개최된|개막|폐막|참석|수상|선정|위촉|추대|"
    # 수치·추세 서술
    r"넘어서|넘어섰|돌파|육박|기록했|기록|차지|앞둬|앞두고|나서|맞아|"
    r"이어져|잇따라|확산|재개|중단|무산|철회|연기|취소했|"
    # 보도 명사 종결 (v7.1) — 와이어 기사 제목은 명사·명사구로 끝나는 경우가 많다.
    #   "…주민 대피령 발령" "…자사주 처분" "…코스피 상장 추진" "…TF 출범"
    #   이걸 안 넣으면 연합뉴스 전재 기사가 통째로 게시판 글로 오분류된다.
    r"발령|전소|소실|대피령|경보|주의보|비상령|"
    r"처분|매각|인수|합병|상장|출범|해산|폐지|신설|개편|"
    r"추진|검토|착수|돌입|강행|보류|유예|재검토|"
    r"의혹|혐의|고발장|진정서|탄원|청원|"
    r"전망|관측|분석|해석|평가|지적|비판론|반론|"
    r"방침|계획|대책|방안|조치|지침|권고|주문|"
    r"우승|준우승|입상|금메달|은메달|동메달|\d+위\b|"
    # 기관·직함 (v7.1) — 보도 대상 주체
    r"대통령|총리|장관|차관|의원|청장|시장|지사|총장|위원장|대변인|"
    r"국회|정부|당국|법원|검찰|경찰|연준|중앙은행|외교부|동포청|"
    r"영사관|총영사관|대사관|한인회장|"
    # 서술 어미 — 평서형 종결(기사체)만. '합니다·있습니다' 정중체는 게시판체라 제외.
    r"했다|한다|된다|됐다|였다|이다|린다|난다|온다|간다|본다|든다|판다|"
    r"[가-힣]나\?|[가-힣]까\?|"
    # 속보·통신사 마커 (v7.1)
    r"^\[[^\]]{1,10}\]|\(종합\d?\)|\[속보\]|\[단독\]|\[AI픽\]|"
    # 인용 부호 — 하한 2글자 (v7.1: "반분" 같은 2글자 인용이 빠지던 문제)
    r'"[^"]{2,}"|“[^”]{2,}”|「[^」]{2,}」',
    re.IGNORECASE)

# 게시판 글 종결 — 혼재 매체에서 이 어미로 끝나면 기사가 아니라 게시글이다.
#   뉴스 제목은 "~했다·~한다"로 끝나고, 게시판 글은 "~합니다·~있습니다·~드려요"로 끝난다.
#   정중체 종결 자체가 게시판 신호이므로 서술어 판정보다 먼저 본다.
BOARD_ENDING_RE = re.compile(
    r"(합니다|함니다|습니다|입니다|해요|드려요|드립니다|please|thanks?)\s*[.!~]*\s*$",
    re.IGNORECASE)


def is_mixed_source(item: dict) -> bool:
    blob = f"{item.get('media','')} {item.get('feed_media','')}".lower()
    return any(m in blob for m in MIXED_SOURCE_MEDIA)


def has_news_predicate(title: str) -> bool:
    return bool(NEWS_PREDICATE_RE.search(_clean_title(title)))
...
def is_excluded(item: dict) -> tuple[bool, str]:
    ...
    # v7 광고 구조 차단 ③(주력): 혼재 매체는 기사만 통과시킨다.
    if is_mixed_source(item):
        clean = _clean_title(title)
        if BOARD_ENDING_RE.search(clean):
            return True, "혼재매체 게시글: 정중체 종결(게시판 화법)"
        if not has_news_predicate(title):
            return True, "혼재매체 비뉴스: 서술어 없음(게시판 글 추정)"
